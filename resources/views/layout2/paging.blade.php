@if(isset($lastPage) && $lastPage!="")
    @php $lastP=$lastPage; @endphp
@else
   @php $lastP=$paginator->lastPage(); @endphp
@endif
@if ($lastP > 1)
    <ul class="pagination">
        <!-- si la pagina actual es distinto a 1 y hay mas de 10 hojas muestro el boton de 1era hoja -->
        <!-- if actual page is not equals 1, and there is more than 5 pages then I show first page button -->
        @if ($paginator->currentPage() != 1 && $lastP >= 5)
            <li>
        ...
        @for($i = max($paginator->currentPage()-2, 1); $i <= min(max($paginator->currentPage()-2, 1)+4,$lastP); $i++)
        ...
        @if ($paginator->currentPage() != $lastP)
            <li>
                <a href="{{ $paginator->url($paginator->currentPage()+1) }}" >
                    >
                </a>
        ...
        @if ($paginator->currentPage() != $lastP && $lastP >= 5)
            <li>
                <a href="{{ $paginator->url($lastP)}}" >
                    >>
                </a>
            </li>
        @endif
