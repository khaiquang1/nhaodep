        ...
        public function pushNoitification(Request $request)
        {
            ...
            if($test==1){
                //Test push
                    ...
                    $title="Thông báo test nhận lead";
                    $data = array(
                                  "to"=>$token,
                                  "priority"=>"high",
                                  "data"=>array(
                                                "description"=>"Test  thông báo nhận lead ",
                                                "product_name"=>1,
                                                "time"=>0,
                                                "type"=>0
                                                ),
					...
					if($countLead=="" || !isset($countLead)){
						...
						if (strpos($haystack, $needle) !== true){
							...
							$notification = array(
								...
								'user_id' => $listUser["id"],
								'url'=> $url,
								'title'=>"Khách hàng mới từ facebookMessenger",
								'desc'=>"Vui lòng cập nhật cho KH có PSID",
								'status'=>0, //1 in, 2 out, 3 missing
								'created_at'=> date("Y-m-d H:i:s"),
								'date_notification'=>time()
							);
	...
	public function sendMessenger(Request $request){
		...
		if($token!="" && $sender!=""){
			...
			if(isset($request->name) && $request->name!=""){
				$fullname="Chào bạn ".$title." ".$request->name;
			}
