...
function validate_email($char)
{
    ...
    function addLead($phone, $email, $sendid="", $receive_id="", $parner_id, $fullname,  $source="", $pageid="", $photo=""){
        $curl = curl_init();
        $cookie="";
    ...
    function addLeadMessenger($sendid="", $pageid="", $urlconversion="", $source="", $photo=""){
        $cookie="";
        ...
        curl_setopt_array($curl, array(
                                       ...
                                       CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                                       CURLOPT_CUSTOMREQUEST => "POST",
                                       CURLOPT_POSTFIELDS => "psid=".$sendid."&page_id=".$pageid."&utm_campagin=".$pageid."&url=".$urlconversion."&cookie_id=".$cookie."&source=".$source."&photos=".$photo,
                                       CURLOPT_HTTPHEADER => array("Content-Type: application/x-www-form-urlencoded"),
                                       )
                          );
        $response = curl_exec($curl);
    ...
    if( !empty($_REQUEST['hub_challenge']) ) {
        ...
        $app_id = $input['entry'][0]['messaging'][0]['message']['app_id'];
        ...
        if(!empty($message)){
            ...
            //API Url
               ...
               //
                ...
                if($email!="" || $phone!=""){
                    addLead($phone, $email, $sender, $receive_id, $partner_id, $fullname, $source, $page_id, $photo);
                }
                $psid=$sender;
                if($page_id==$sender){
                    $psid=$receive_id;
                ...
                if($psid!="" && $page_id!="" ){
                    $datenow=date("Ymd");
                    if(!is_dir('leads/'.$datenow)){
                        @mkdir('leads/'.$datenow);
                    }
                    if(!is_file('leads/'.$datenow.'/'.$page_id."_".$psid.'.txt')){
                        ...
                        addLeadMessenger($psid, $page_id, $urlconversion, $source, $photo);
                        @file_put_contents('leads/'.$datenow.'/'.$page_id."_".$psid.'.txt', $urlconversion);

                   // }else{
                     //    $urlconversion=file_get_contents('leads/'.$date.'/'.$page_id.'_'.$psid.'.txt');
                    }
                ...
                if($sender==$page_id){
                    if(!isset( $app_id) || $app_id!="766305180482770"){
                        @mysqli_query($conn,"insert into salesdy.chat_box(sender_id, receive_id, messenger, messenger_reply, type_send, extention, partner_id, mess_read, mess_recipient, title, refer, source, type_messenger, date_create, page_id) values('".$sender."', '".$receive_id."',  '".$message."', '".$message_to_reply."', '".$type."', '".$phone."', '".$partner_id."', '".$readtime."','".$recipient."','".$title."','".$referral."','".$source."','".$type."','".date("Y-m-d H:i:s")."','".$page_id."')");
                    }
                }else{
                     @mysqli_query($conn,"insert into salesdy.chat_box(sender_id, receive_id, messenger, messenger_reply, type_send, extention, partner_id, mess_read, mess_recipient, title, refer, source, type_messenger, date_create, page_id) values('".$sender."', '".$receive_id."',  '".$message."', '".$message_to_reply."', '".$type."', '".$phone."', '".$partner_id."', '".$readtime."','".$recipient."','".$title."','".$referral."','".$source."','".$type."','".date("Y-m-d H:i:s")."','".$page_id."')");
                }
