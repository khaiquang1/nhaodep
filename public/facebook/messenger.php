...
function addLead($phone, $email, $sendid="", $receive_id="", $parner_id, $fullname){
    $curl = curl_init();
    curl_setopt_array($curl, array(
                                   ...
                                   CURLOPT_POSTFIELDS => "phone=".$phone."&email=".$email."&fullname=".$fullname."&sender=".$receive_id."&partner_id=".$parner_id."&utm_source=FacebookMessenger&utm_medium=".$sendid."&utm_campagin=FacebookPage&timestamp=".time()."&url=&utm_content=&utm_term=&GCLID=&FBCLID=&PID=&PSID=&Token=".$Token."&cookie_id=",
    ...
    function addLeadMessenger($sendid="", $pageid="", $urlconversion=""){
        $curl = curl_init();
        curl_setopt_array($curl, array(
                                       ...
                                       CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                                       CURLOPT_CUSTOMREQUEST => "POST",
                                       CURLOPT_POSTFIELDS => "psid=".$sendid."&page_id=".$pageid."&utm_campagin=".$pageid."&url=".$urlconversion,
                                       CURLOPT_HTTPHEADER => array("Content-Type: application/x-www-form-urlencoded"),
                                       )
                          );
        $response = curl_exec($curl);
...
if( !empty($_REQUEST['hub_challenge']) ) {
    ...
    $token="";
    @file_put_contents('chatlog/'.$page_id.'.txt', json_encode($input));
    ...
    if(!empty($message)){
        ...
        //API Url
           ...
           //
            ...
            if($email!="" || $phone!=""){
                addLead($phone, $email, $sender, $receive_id, $partner_id, $fullname);
            }
            $psid=$sender;
            if($page_id==$sender){
                $psid=$receive_id;
            }
            if($psid!="" && $page_id!="" && !is_file('lead/'.$page_id."_".$psid.'.txt')){
                ...
                addLeadMessenger($psid, $page_id,$urlconversion);
                @file_put_contents('lead/'.$page_id."_".$psid.'.txt', "https://graph.facebook.com/v6.0/".$page_id."/conversations?user_id=".$psid."&access_token=".$token);

            }
            @mysqli_query($conn,"insert into messenger(sender_id, receive_id, messenger, messenger_reply, type_send, extention, partner_id, mess_read, mess_recipient, title) values('".$sender."', '".$receive_id."', '".$message."', '".$message_to_reply."', '".$type."', '".$phone."', '".$partner_id."', '".$readtime."','".$recipient."','".$title."')");
