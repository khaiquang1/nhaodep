<?php
      ...
      foreach ($accountsEdge as $account) {
          ...
          if(!empty($ac['access_token']) ){
              ...
              curl_setopt_array($curl, array(
                  ...
                  CURLOPT_URL => 'https://graph.facebook.com/v7.0/'.$page_id.'/subscribed_apps?access_token='.$page_access_token.'&subscribed_fields=messages,messaging_postbacks,messaging_optins,message_reads,messaging_referrals,inbox_labels,message_deliveries',
  ...
  if($listinsert!="" && count($listinsert)>0){
    @mysqli_query($conn,"insert into page_partner(title, partner_id, page_id, token) values".implode(",",$listinsert)." on DUPLICATE KEY UPDATE partner_id = VALUES (partner_id), page_id=VALUES (page_id)");
    @mysqli_close($conn);
  }
